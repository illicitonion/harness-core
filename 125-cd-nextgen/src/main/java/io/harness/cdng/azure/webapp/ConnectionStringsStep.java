...
public class ConnectionStringsStep implements SyncExecutable<ConnectionStringsParameters> {
  ...
  public StepResponse executeSync(Ambiance ambiance, ConnectionStringsParameters stepParameters,
    ...
    validateStoreReferences(stepParameters.getConnectionStrings(), ambiance);
  ...
  private void validateFileRefs(HarnessStore harnessStore, Ambiance ambiance) {
    ...
    if (isEmpty(fileReferences)) {
      throw new InvalidRequestException(
          format("Cannot find any file reference for connection strings, store kind: %s", harnessStore.getKind()));
    }
    if (fileReferences.size() > 1) {
      throw new InvalidRequestException(
          format("Only one connection strings file should be provided, store kind: %s", harnessStore.getKind()));
    }
  ...
  private void validateFileByRef(HarnessStore harnessStore, Ambiance ambiance, HarnessStoreFile file) {
    if (ParameterField.isNull(file.getRef())) {
      throw new InvalidRequestException(
          format("File ref not found for one for connection strings, store kind: %s", harnessStore.getKind()));
    }
    ...
    if (ParameterFieldHelper.getBooleanParameterFieldValue(file.getIsEncrypted()) == Boolean.TRUE) {
      ...
      if (ngEncryptedData == null) {
        throw new InvalidRequestException(
            format("Connection strings file not found in Encrypted Store with ref: [%s]", file.getRef().getValue()));
      }
    } else {
      Optional<FileStoreNodeDTO> connectionStrings = fileStoreService.get(fileRef.getAccountIdentifier(),
          fileRef.getOrgIdentifier(), fileRef.getProjectIdentifier(), fileRef.getIdentifier(), false);

      if (!connectionStrings.isPresent()) {
        throw new InvalidRequestException(
            format("Connection strings file not found in File Store with ref: [%s]", file.getRef().getValue()));
      }
    }
  }
  ...
  private void validateConnectorByRef(StoreConfig storeConfig, Ambiance ambiance) {
    if (ParameterField.isNull(storeConfig.getConnectorReference())) {
      throw new InvalidRequestException(
          format("Connector ref field not present in connection strings, store kind: %s ", storeConfig.getKind()));
    }
