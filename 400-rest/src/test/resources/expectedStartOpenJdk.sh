...
ACCOUNT_STATUS=$(curl $MANAGER_PROXY_CURL -s https://localhost:9090/api/account/ACCOUNT_ID/status | cut -d ":" -f 3 | cut -d "," -f 1 | cut -d "\"" -f 2)
...
REMOTE_WATCHER_LATEST=$(curl $MANAGER_PROXY_CURL -s $WATCHER_STORAGE_URL/watcherci.txt)
...
DELEGATE_STORAGE_URL=http://localhost:8888
  REMOTE_DELEGATE_LATEST=$(curl $MANAGER_PROXY_CURL -s $DELEGATE_STORAGE_URL/delegateci.txt)
