...
public class AzureWebAppTaskNG extends AbstractDelegateRunnableTask {
  ...
  private void decryptRequest(AzureWebAppTaskRequest webAppTaskRequest) {
    webAppTaskRequest.fetchDecryptionDetails().forEach((decryptableEntity, encryptedDataDetails) -> {
      decryptionService.decrypt(decryptableEntity, encryptedDataDetails);
