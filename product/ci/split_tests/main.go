...
// It doesn't necessarily have time units. For example, we could split the files
// based on filesize or lines of code in which case the time field indicates lines.
...
// Calculate file times based on number of lines of code in the file. This can be a simple proxy
// for splitting files when timing data is not available.
...
// Remove files that don't exist from the fileTimes map.
...
func processFiles(fileTimesMap map[string]float64, currentFileSet map[string]bool) {
	...
	averageFileTime := 1.0     // Default time for a file
